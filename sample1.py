...
# Function to load a template image, compute its keypoints and descriptors
...
def feature_match(img, templates):
    ...
    for template, kp1, des1, label in templates:
        ...
        # Lowe's ratio test with a stricter threshold
        good_matches = []
        for m, n in matches:
            if m.distance < 0.60 * n.distance:  # Stricter Lowe's ratio
                good_matches.append(m)

        # Update best match if current one is better
        if len(good_matches) > max_matches and len(good_matches) > 15:  # Higher threshold for matches
...
while True:
    ...
    cv2.imshow('SIFT Feature Matching for Multiple Images', frame)
